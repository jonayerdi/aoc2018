...
fn parse_map(map: &str) -> (HashMap<Position, Connections>, Vec<Cart>) {
...
fn find_crashes(carts: &[Cart]) -> HashSet<Position> {
...
fn main() {
    ...
    loop {
        ...
        let crashes = find_crashes(&carts);
        if !crashes.is_empty() {
            crashes.iter().for_each(|position| {
                println!(
        ...
        carts.iter_mut().for_each(|cart| cart.next_step(&tracks));
        step += 1;
    }
}
